...
namespace Telphone
{
    internal class Accounts
    {
        ...
        static public List<string> ComboboxValue(NpgsqlConnection connection, string str)
        {
            ...
            while (reader.Read())
            {
                status.Add(reader.GetInt32(0).ToString());
            }
            connection.Close();
            return status;
        }
        static public List<string> ComboBigInt(NpgsqlConnection connection, string str)
        {
            connection.Open();
            var status = new List<string>();
            NpgsqlCommand command = new NpgsqlCommand(str, connection);
            ...
            while (reader.Read())
            {
                status.Add(reader.GetInt64(0).ToString());
            }
            connection.Close();
            return status;
        }
        static public List<string> ComboString(NpgsqlConnection connection, string str)
        {
            connection.Open();
            var status = new List<string>();
            NpgsqlCommand command = new NpgsqlCommand(str, connection);
